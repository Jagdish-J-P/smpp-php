...
class SmppClient
{
    ...
    public function bindTransreceiver($login, $pass)
    {
        ...
        if ($this->debug) {
            call_user_func($this->debugHandler, 'Binding transreceiver...');
        }
    ...
    public function sendSMS(SmppAddress $from, SmppAddress $to, $message, $tags = null, $dataCoding = SmppConstants::DATA_CODING_DEFAULT, $priority = 0x00, $scheduleDeliveryTime = null, $validityPeriod = null)
    {
        ...
        if ($msg_length > 160 && $dataCoding != SmppConstants::DATA_CODING_UCS2 && $dataCoding != SmppConstants::DATA_CODING_DEFAULT) {
            return false;
        }
        ...
        if ($doCsms) {
            ...
            foreach ($parts as $part) {
                $sartags = [$sar_msg_ref_num, $sar_total_segments, new SmppTag(SmppTag::SAR_SEGMENT_SEQNUM, $seqnum, 1, 'c')];
    ...
    protected function submit_sm(SmppAddress $source, SmppAddress $destination, $short_message = null, $tags = null, $dataCoding = SmppConstants::DATA_CODING_DEFAULT, $priority = 0x00, $scheduleDeliveryTime = null, $validityPeriod = null)
    {
        ...
        try {
            ...
            elseif ($response->status === SmppConstants::ESME_ROK) {
    ...
    protected function getCsmsReference()
    {
        if (! isset($this->sar_msg_ref_num)) {
            $this->sar_msg_ref_num = mt_rand(0, 65535);
        }
        ++$this->sar_msg_ref_num;

        if ($this->sar_msg_ref_num > 65535) {
    ...
    protected function _bind($login, $pass, $command_id)
    {
        ...
        if ($response->status != SmppConstants::ESME_ROK) {
            throw new SmppException(SmppConstants::getStatusMessage($response->status), $response->status);
        }
    ...
    {
        // Check command id
        if ($pdu->id != SmppConstants::DELIVER_SM) {
    ...
    {
        return $this->sendCommand(SmppConstants::ENQUIRE_LINK, null);
    }
    ...
    {
        return $this->readPDU_resp(2, SmppConstants::SUBMIT_SM_RESP);
    }
    ...
    protected function reconnect()
    {
        ...
        elseif ($this->mode == 'transreceiver') {
    ...
    protected function readPDU_resp($seq_number, $command_id)
    {
        ...
        do {
            ...
            if ($pdu) {
                if ($pdu->sequence == $seq_number && ($pdu->id == $command_id || $pdu->id == SmppConstants::GENERIC_NACK)) {
                    return $pdu;
                }

                if ($pdu->sequence == null && $pdu->id == SmppConstants::GENERIC_NACK) {
                    return $pdu;
                }
